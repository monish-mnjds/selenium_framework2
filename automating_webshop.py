...
element = read_xl.read_excel(WORK_BOOK, SHEET_NAME)
web_utls = Lib()

firstname, lastname, email, password, c_password = read_xl.read_test_data(TEST_PATH, TEST_SHEET, "tc01")
...
class AutomatingWebShop:
    ...
    def enter_first_name(self):
        ...
        web_utls.sendKeys(self.driver, element["enterFirstname"], firstname)

    def enter_last_name(self):
        # driver.find_element_by_id("LastName").send_keys("Kohli")
        web_utls.sendKeys(self.driver, element["enterLastname"], lastname)

    def enter_email(self):
        # driver.find_element_by_xpath("//input[@id='Email']").send_keys("viratkohli18@gmail.com")
        web_utls.sendKeys(self.driver, element["enterEmail"], email)

    def enter_password(self):
        # driver.find_element_by_xpath("//input[@id='Password']").send_keys("viratvirat18")
        web_utls.sendKeys(self.driver, element["enterPassword"], password)

    def confirm_password(self):
        # driver.find_element_by_xpath("//input[@id='ConfirmPassword']").send_keys("viratvirat18")
        web_utls.sendKeys(self.driver, element["confirmPassword"], c_password)
